...
## Industrial data sources are first-class

A generic AI tool, asked to build a dashboard, will reach for what it knows: a REST API in front of a fresh database schema invented at prompt time. That works for a SaaS internal tool because the data lives in a database the AI just created. On the factory floor the data already exists - in a PLC sitting next to the line, in an SAP system three buildings over, in an MQTT broker on the IoT VLAN, in a SQL Server in the IT room, in an OPC UA server that already aggregates half the shop. Asking a generic vibe coder to wire that up is asking it to invent industrial protocols it has never seen and bridge them through code it has never tested. Peakboard's data source library knows about OPC UA, MQTT, SAP RFC, S7, Modbus, REST, SQL and dozens more out of the box. When the prompt says "show OEE for line 3", the AI does not build a database - it picks the existing OPC UA tags, wires them to a tile, and produces a project that talks directly to the machine that already has the answer.
...
- **Industrial data sources are first-class.** OPC UA, MQTT, SAP, SQL, S7 and the rest are built in. The AI wires up against the systems already on the floor instead of inventing a database for every project.
