...
We don't want visitors to see the Guacamole login screen. Instead, we build a simple HTML page that authenticates against the Guacamole API behind the scenes and immediately loads the RDP session in a full-viewport iframe. The page grabs a session token from the Guacamole REST API, looks up the first available connection, and constructs the client URL:
...
async function connect() {
    ...
    document.getElementById("rdp-frame").src =
        "/guacamole/#/client/" + clientId
        + "?token=" + data.authToken;
}
{% endhighlight %}

The client identifier is a Base64-encoded string combining the connection ID, the connection type (`c` for connection), and the data source (`mysql`). Guacamole uses a URL-safe Base64 variant, hence the character replacements.
